...
const props = defineProps({
    ...
    isLate: Boolean, // New prop to identify if it's a late report
});
...
const form = useForm({
    execution_date: getTodayForInput(),
    location: '',
    pic: '',
    position: '',
    result_description: '',
...
watch(() => props.plan, (newPlan) => {
    if (newPlan) {
        // Reset or init form
        form.execution_date = getTodayForInput();
        form.location = '';
        form.pic = '';
        form.position = '';
...
<template>
    <Modal :show="show" @close="$emit('close')" maxWidth="2xl">
        <div class="font-sans">
            <!-- Header -->
            <div :class="isLate ? 'bg-amber-500' : 'bg-blue-600'" class="px-4 py-4 sm:px-5 sm:py-4 flex items-center justify-between rounded-t-lg transition-colors duration-300">
                <div class="pr-8 sm:pr-0">
                    <h2 class="text-base sm:text-lg font-bold text-white leading-tight">
                        {{ isLate ? 'Late Report' : 'Create Report' }}
                    </h2>
                    <p :class="isLate ? 'text-amber-100' : 'text-blue-100'" class="text-[9px] sm:text-xs italic" v-if="plan">
                        For: {{ companyName || plan?.customer?.company_name || 'N/A' }} | {{ plan?.project_name || 'No Project' }}
                    </p>
                </div>
                <button @click="$emit('close')" 
                    :class="isLate ? 'text-amber-100 hover:bg-amber-400/50' : 'text-blue-100 hover:bg-blue-500/50'"
                    class="p-2 -mr-2 text-white transition-colors rounded-full">
            ...
            <form @submit.prevent="submit" class="max-h-[85vh] overflow-y-auto overflow-x-hidden">
                ...
                <div class="px-5 py-4 bg-gray-50/50 flex flex-col sm:flex-row items-center justify-end gap-3 rounded-b-lg border-t border-gray-100">
                    ...
                    <button type="submit" :disabled="form.processing"
                        :class="[
                            'w-full sm:w-auto inline-flex items-center justify-center gap-2 px-8 py-2.5 text-white text-sm font-bold rounded-lg shadow-md disabled:opacity-50 disabled:cursor-not-allowed order-1 sm:order-2 transition-colors',
                            isLate ? 'bg-amber-600 hover:bg-amber-700' : 'bg-blue-600 hover:bg-blue-700'
                        ]">
